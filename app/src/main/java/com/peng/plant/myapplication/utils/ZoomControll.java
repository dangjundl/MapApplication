package com.peng.plant.myapplication.utils;
...
public class ZoomControll extends FrameLayout {
...
//       this.ch = ch;
    ...
    public boolean dispatchTouchEvent(final MotionEvent ev) {
        ...
        Log.d(TAG, "dispatchTouchEvent: ev값 : " + ev);
...
//        invalidate();
    ...
    private void processSingleTouchOutsideMinimap(final MotionEvent ev) {
        ...
        touchLastX = x;
        touchLastY = y;
        Log.d(TAG, "touchLastX의 값 " + touchLastX);
        Log.d(TAG, "touchLastY 값 " + touchLastY);
            ...
            case MotionEvent.ACTION_MOVE:
                if (scrolling || (smoothZoom > 1.0f && l > 30.0f)) {
                    if (!scrolling) {
                        ...
                        Log.d(TAG, "processSingleTouchOutsideMinimap: ddd: " + super.dispatchTouchEvent(ev));
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                if (pinching || ld > 30.0f) {
                    ...
                    Log.d(TAG, "zoomX 정보: " + zoomX);
                    Log.d(TAG, "zoomY 정보: " + zoomY);
                    Log.d(TAG, "dxk 정보: " + dxk);
                    Log.d(TAG, "dyk 정보: " + dyk);

                    Log.d(TAG, "processDoubleTouchEvent: sds" + Math.max(1.0f, zoom * d / (d - dd))); // 줌당기는 값
...
//            p.setColor(0x80000000 | 0x00ffffff & miniMapColor);
            //미니맵 가로
            final int w = (int) (miniMapHeight * (float) getWidth() / getHeight() / 2); //* (float) getWidth() / getHeight();
//            final int w = (int) (ch.getWidth() *  (float) getWidth() / getHeight() /2); //* (float) getWidth() / getHeight();

            //미니맵 세로
            final int h = (int) (miniMapHeight / 2);
            canvas.translate(30, 30); //화면에 생성되는 위치

            Bitmap resize_bitmap = Bitmap.createScaledBitmap(v.getDrawingCache(), w, h, true);

            canvas.drawBitmap(resize_bitmap, 0, 0, p);
...
//            p.setAlpha(100);  // 투명도 조정
            ...
            Log.d(TAG, "dispatchDraw: zoomx " + zoomX);
            Log.d(TAG, "dispatchDraw: zoomy " + zoomY);

            final float fix_dx = (float) (w * 427.0 / getWidth()); //검은테두리 가로 (
            final float fix_dy = (float) (h * 240.0 / getHeight()); //검은 테두리 세로
...
//            canvas.setBitmap(ch);
//            canvas.translate(100.0f, 10.0f); //화면에 생성되는 위치
        }
